...
class Randoms(Cog):
    ...
    async def dice(self, ctx: Context, rolls: int = 1) -> None:
        ...
        if 1 < rolls < 100:
            s = round(np.sum(random.sample(range(1, 6), rolls)) / rolls, 3)
    ...
    @command(aliases=("pick", "choice"))
    async def choose(self, ctx: Context, *choices) -> None:
        """Returns only one of the items that the user gives."""
        embed = DefaultEmbed(ctx, description=f"A choice was made. Fate landed on {random.choice(choices)}.")
        await ctx.send(embed=embed)
