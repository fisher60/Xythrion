...
class Context(commands.Context):
    ...
    async def check_for_subcommands(self) -> None:
        ...
        if self.invoked_subcommand is None:
            ...
            await self.embed(description=error_string)

    async def embed(self, *, description: str, buffer: Optional[BytesIO] = None) -> None:
        """Creating then sending embeds."""
        startup_delta = naturaldelta(datetime.now() - self.bot.startup_time)

        description = f'`{description}`' if '\n' not in description else description
